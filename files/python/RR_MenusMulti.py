...
try:
    from files.RR_Utils import MyHTMLParser
except ImportError:
    from RR_Utils import MyHTMLParser
...
class VersusFen(Tk):
    def __init__(self):
        ...
        if len(self.result) >= 7:
            self.nbPage = int(len(self.result)/9)+1
        else:
            self.nbPage = 1
...
class CommunityFen(Tk):
    def __init__(self):
        ...
        backgroundImage = ImageTk.PhotoImage(
            Image.open("files/fond_intro.jpg"))
        canvas.create_image(width/2, height/2, image=backgroundImage)
        canvas.create_text(330, 30, text="Choix du niveau", font=("Comic Sans MS", 20, "bold"), fill='#FA6C00')
        canvas.grid(row=1, column=1, columnspan=3, rowspan=5)
        self.page = 1
        self.listeButton = []
