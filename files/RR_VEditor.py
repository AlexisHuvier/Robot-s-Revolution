...
class VEditor(Tk):
    ...
    def __init__(self, level, mode, ia = None):
        ...
        self.difficult = "MP"
        self.aide = ""
        self.blockInit = [["walk", ImageTk.PhotoImage(Image.open("files/bloc.png"))],
                        ["left", ImageTk.PhotoImage(Image.open("files/bloc.png"))],
                        ["right", ImageTk.PhotoImage(Image.open("files/bloc.png"))],
                        ["jump", ImageTk.PhotoImage(Image.open("files/bloc.png"))]
        ]
    ...
    def selectBlock(self, evt):
        ...
        if evt.x >= 509 and evt.x <= 642:
            ...
            if len(self.blockList) > 1:
                ...
                if len(self.blockList) > 2:
                    self.blockList[len(self.blockList)-2] = [self.blockList[len(self.blockList)-2][0],ImageTk.PhotoImage(Image.open("files/blocF.png"))]
        ...
        if bloc != "":
            if self.title()[0] != "*":
                self.title("*"+self.title())
            if len(self.blockList) == 1:
                self.blockList.append([bloc, ImageTk.PhotoImage(Image.open("files/blocF.png"))])
            else:
                self.blockList[len(self.blockList)-1] = [self.blockList[len(self.blockList)-1][0],ImageTk.PhotoImage(Image.open("files/blocM.png"))]
                self.blockList.append([bloc, ImageTk.PhotoImage(Image.open("files/blocF.png"))])
            self.showBlocks()
    ...
    def showBlocks(self):
        self.bgVisuel=ImageTk.PhotoImage(Image.open("files/visuel.png"))
        self.show.create_image(463, 312, image = self.bgVisuel)
        self.poub=ImageTk.PhotoImage(Image.open("files/poubelle.png"))
    ...
    def convertToVisuel(self, content):
        ...
        for i in range(len(listContent)):
            if i == len(listContent)-1:
                self.blockList.append([listContent[i].split("(")[0], ImageTk.PhotoImage(Image.open("files/blocF.png"))])
            else:
                self.blockList.append([listContent[i].split("(")[0], ImageTk.PhotoImage(Image.open("files/blocM.png"))])
